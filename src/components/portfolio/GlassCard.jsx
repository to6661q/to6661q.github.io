...
export default function GlassCard({ children, delay = 0, className = "" }) {
  ...
  return (
    <motion.div
      ...
      style={{
        backgroundColor: 'rgba(0, 0, 0, 0.1)', // ← 90% TRANSPARAN (hanya 10% hitam)
        backdropFilter: 'blur(24px)',
        WebkitBackdropFilter: 'blur(24px)',
        border: '1px solid rgba(255, 255, 255, 0.15)', // ← Border sedikit lebih terang agar terlihat
      }}
      onMouseEnter={(e) => {
        e.currentTarget.style.backgroundColor = 'rgba(0, 0, 0, 0.2)'; // ← Hover: 80% transparan
        e.currentTarget.style.borderColor = 'rgba(17, 169, 0, 0.5)';
        e.currentTarget.style.boxShadow = '0 0 20px rgba(17, 169, 0, 0.15)';
      }}
      onMouseLeave={(e) => {
        e.currentTarget.style.backgroundColor = 'rgba(0, 0, 0, 0.1)'; // ← Kembali 90% transparan
